  ...
  if (!scenario) {
    return (
      <Box sx={{ minHeight: "100vh", bgcolor: "#fafafa", display: "flex", flexDirection: "column", alignItems: "center", px: 2, py: { xs: 5, md: 8 } }}>
        ...
        {STATIC_DEMO_MODE && (
          <Alert severity="info" sx={{ width: "100%", maxWidth: 440, mb: 3, borderRadius: "14px" }}>
            当前为静态演示版，截图分析会展示本地生成的示例结论，用来演示完整交互流程。
          </Alert>
        )}
        <Stack spacing={2} sx={{ width: "100%", maxWidth: 440 }}>
          {([
            { val: "pre_publish" as Scenario, title: "发布前分析（示意）", desc: "内容打磨阶段 — 展示静态版分析流程", color: "#2563eb" },
            { val: "post_publish" as Scenario, title: "发布后分析（示意）", desc: "已发布内容 — 展示静态版复盘流程", color: "#16a34a" },
          ]).map((s) => (
            <Box
              key={s.val}
