...
import { quickRecognize, quickRecognizeVideo, getApiHealth, STATIC_DEMO_MODE } from "../utils/api";
...
export default function Home() {
  ...
  const processingStatus = useMemo(() => {
    ...
    if (uploadingPulse) {
      return { label: "上传中", tone: "info" as const, text: STATIC_DEMO_MODE ? "素材已接收，正在生成演示预览..." : "素材已接收，正在准备识别..." };
    }
    if (pendingRecognition) {
      ...
      if (videoPending && !imagePending && imageFileKeys.size === 0) {
        return { label: "识别中", tone: "info" as const, text: STATIC_DEMO_MODE ? "正在生成视频示例识别结果，请稍候..." : "AI 正在识别视频内容（含画面与字幕），请稍候..." };
      }
      return { label: "识别中", tone: "info" as const, text: STATIC_DEMO_MODE ? "正在生成示例识别结果..." : "AI 正在自动识别图片与视频..." };
    }
    if (analyzingPulse) {
      return { label: "分析中", tone: "info" as const, text: STATIC_DEMO_MODE ? "正在汇总示例结果并回填表单..." : "正在汇总识别结果并回填表单..." };
    }
    if (allRecognitionDone) {
      return { label: "已就绪", tone: "success" as const, text: STATIC_DEMO_MODE ? "素材已就绪，可以进入演示诊断。" : "识别完成，可以继续发起诊断。" };
    }
    return null;
  }, [
    files.length,
  ...
  const handleSubmit = () => {
    ...
    if (lockInputs || isFormBlocked) { setSubmitError(STATIC_DEMO_MODE ? "示例识别生成中，请稍等" : "AI 识别中，请稍等"); return; }
    setSubmitError("");
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: { xs: 0.75, md: 1.5 } }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 0.75, flexShrink: 0 }}>
            <Box sx={{
              ...
              <Typography sx={{ color: "#fff", fontSize: 10, fontWeight: 800, fontFamily: "Inter" }}>毒</Typography>
            </Box>
          ...
          <Typography sx={{
            ...
            {STATIC_DEMO_MODE ? "静态演示版：支持直接分享，识别和报告为本地示意结果" : "基于大量数据训练用户画像和流量预测模型"}
          </Typography>
        </Box>
        <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
          <Button
            onClick={() => navigate("/anti-shit")}
            size="small"
          ...
          <Box sx={{
            ...
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
              <Box>
                ...
                <Typography sx={{ fontSize: 12, color: "#999", mt: 0.25 }}>
                  {STATIC_DEMO_MODE ? "上传图片或视频，体验完整演示流程" : "把小红书截图拖进来，AI 自动识别标题、正文、分类"}
                </Typography>
              </Box>
              {files.length > 0 && (
            ...
            {STATIC_DEMO_MODE ? (
              <Alert severity="info" sx={{ fontSize: 12, py: 0.5, borderRadius: "10px" }}>
                当前为可分享的静态演示版。你仍然可以上传素材、填写表单并体验完整流程，诊断结果会由前端本地生成。
              </Alert>
            ) : apiReachable === false && (
            ...
            {isReady && files.length > 0 && hasRecogSuccess && (
              <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, px: 0.5 }}>
                ...
                <Typography sx={{ fontSize: 11, color: "#16a34a", fontWeight: 600 }}>
                  {STATIC_DEMO_MODE ? "示例识别完成，可以查看演示报告" : "分析完成，可以开始诊断"}
                </Typography>
              </Box>
            )}
            {allFailed && (
            ...
            {isFormBlocked && (
              <Box sx={{ display: "flex", alignItems: "center", gap: 0.75, px: 1, py: 0.5, borderRadius: "8px", bgcolor: "#eff6ff", flexShrink: 0 }}>
                ...
                <Typography sx={{ fontSize: 12, color: "#3b82f6", fontWeight: 500 }}>
                  {STATIC_DEMO_MODE ? "正在生成示例识别结果，完成后自动填入" : "AI 识别中，完成后自动填入"}
                </Typography>
              </Box>
            )}
            ...
            <Box sx={{
              ...
              <Box>
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, mb: 0.5 }}>
                  ...
                  {autoFilled.title && <Typography sx={{ fontSize: 10, color: "#16a34a", fontWeight: 600 }}>{STATIC_DEMO_MODE ? "已填" : "AI 已填"}</Typography>}
                </Box>
              ...
              <Box>
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, mb: 0.5 }}>
                  ...
                  {autoFilled.content && <Typography sx={{ fontSize: 10, color: "#16a34a", fontWeight: 600 }}>{STATIC_DEMO_MODE ? "已填" : "AI 已填"}</Typography>}
                </Box>
              ...
              <Box>
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, mb: 0.75 }}>
                  ...
                  {autoFilled.category && <Typography sx={{ fontSize: 10, color: "#16a34a", fontWeight: 600 }}>{STATIC_DEMO_MODE ? "已识别" : "AI 已识别"}</Typography>}
                </Box>
