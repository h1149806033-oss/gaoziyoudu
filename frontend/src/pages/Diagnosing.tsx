...
/* ── Steps ── */
const STEPS = [
  { label: "生成预评分", desc: "根据输入内容给出首轮示意评分" },
  { label: "整理笔记内容", desc: "提取标题、正文与素材信息" },
  { label: "分析封面视觉", desc: "评估构图、色彩与信息重点" },
  { label: "对比示意基线", desc: "与同类内容的示意样本对比" },
  { label: "组织内容观点", desc: "归纳文案结构与信息密度判断" },
  { label: "组织视觉观点", desc: "归纳封面与首屏吸引力判断" },
  { label: "组织增长观点", desc: "归纳发布策略与标签判断" },
  { label: "组织用户反馈", desc: "生成示意评论与互动反应" },
  { label: "汇总专家讨论", desc: "整理示意专家之间的补充与争议" },
  { label: "生成综合结论", desc: "汇总主要问题与优化重点" },
  { label: "输出演示报告", desc: "生成可浏览的静态诊断结果" },
];
...
export default function Diagnosing() {
  ...
  useEffect(() => {
    ...
    const shouldUseFallbackReport = (message: string) => {
            ...
            if (event.type === "pre_score") {
              ...
              const msg =
                typeof event.data?.message === "string"
                  ? event.data.message
                  : "服务端诊断失败";
              if (shouldUseFallbackReport(msg)) {
                finishWithFallback();
              } else {
